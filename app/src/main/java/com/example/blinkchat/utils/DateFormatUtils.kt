...
fun Date.formatAsYesterday(c: Context): String {
    return c.getString(string.yesterday)
}
...
fun Date.formatAsWeekDay(c: Context): String {
    ...
    return when (calendar.get(DAY_OF_WEEK)) {
        MONDAY -> s(string.monday)
        TUESDAY -> s(string.tuesday)
        WEDNESDAY -> s(string.wednesday)
        THURSDAY -> s(string.thursday)
        FRIDAY -> s(string.friday)
        SATURDAY -> s(string.saturday)
        SUNDAY -> s(string.sunday)
        else -> {
...
fun Date.formatAsHeader(context: Context): String {
    return when {
        isToday() -> context.getString(string.today)
        isYesterday() -> formatAsYesterday(context)
        isThisWeek() -> formatAsWeekDay(context)
        isThisYear() -> {
